...
typedef struct {
  ...
  bool is_nvl_telemetry;
  uint32_t channel_id;
} ncclStatsProfilerPluginConnection_v4;
...
typedef union {
  ...
  ncclStatsTCPConnection tcp_conn;
  ncclStatsRDMAConnection rdma_conn;
  ncclStatsProfilerPluginConnection_v4 profiler_conn;
} ncclStatsConnection_v4;
...
typedef struct {
  ...
  const char* gpu_pci_addr;
  const char* gpu_uuid;
  ncclStatsConnection_v4 connection;
} ncclStatsConnectionIdentifier_v4;
